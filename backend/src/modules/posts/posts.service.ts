...
const IMAGE_UPLOAD_FOLDER = 'post-images/';
...
export class PostsService {
  ...
  public async createPost(data: CreatePostData, userId: string) {
    ...
    if (img) {
      imagePath = `${IMAGE_UPLOAD_FOLDER}${crypto.randomUUID()}`;
      await this.s3Service.putObject(IMAGE_UPLOAD_BUCKET, imagePath, img);
    }

    const newPost = new Post({ user: userId, text, img: imagePath });
  ...
  public async commentOnPost(postId: string, data: CommentPostData, userId: string) {
    ...
    return Post.findByIdAndUpdate(post._id, { $push: { comments: { text: data.text, user: userId } } });
  }
